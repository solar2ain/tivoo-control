...
def _draw_gear(p, frame=0):
    """Draw a small gear icon top-right."""
    g = (180, 180, 180)
    cx, cy = 13, 2
    p[cy][cx] = g; p[cy - 1][cx] = g; p[cy + 1][cx] = g
    p[cy][cx - 1] = g; p[cy][cx + 1] = g
    if frame == 1:
        p[cy - 1][cx - 1] = g; p[cy + 1][cx + 1] = g
    else:
        p[cy - 1][cx + 1] = g; p[cy + 1][cx - 1] = g


def _draw_code_brackets(p):
    """Draw </> code symbol top-right."""
    c = (0, 200, 255)
    # <
    p[1][10] = c; p[2][9] = c; p[3][10] = c
    # /
    p[1][12] = c; p[2][11] = c; p[3][11] = c
    # >
    p[1][13] = c; p[2][14] = c; p[3][13] = c


def _draw_checkmark(p):
    """Draw green checkmark top-right."""
    g = (50, 220, 50)
    p[1][13] = g; p[2][12] = g; p[2][13] = g; p[3][11] = g; p[3][12] = g
    p[1][14] = g
...
def _draw_warning(p, frame=0):
    ...
    r = _RED
    if frame == 0:
        p[1][13] = y; p[2][12] = y; p[2][14] = y; p[3][11] = y; p[3][12] = y; p[3][13] = y; p[3][14] = y; p[3][15] = y
        p[2][13] = r
    else:
        p[1][13] = r; p[2][12] = r; p[2][14] = r; p[3][11] = r; p[3][12] = r; p[3][13] = r; p[3][14] = r; p[3][15] = r
        p[2][13] = y


def _draw_loading(p, frame=0):
    """Draw rotating loading dots around face."""
    w = _WHITE
    d = (100, 100, 100)
    dots = [(0, 7), (0, 8), (3, 14), (7, 15), (12, 14), (15, 8), (15, 7), (12, 1), (7, 0), (3, 1)]
    for i, (y, x) in enumerate(dots):
        if 0 <= y < 16 and 0 <= x < 16:
            p[y][x] = w if (i % len(dots)) == (frame % len(dots)) or ((i - 1) % len(dots)) == (frame % len(dots)) else d
...
def working():
    ...
    f3 = _face_base()
    _draw_eyes(f3, "focused")
    _draw_mouth(f3, "determined")
    _draw_sweat(f3, "both")
    _draw_gear(f3, frame=1)
